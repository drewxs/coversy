...
export const AdminTickets = () => {
  ...
  const resolvedTickets = useSelector((state) => state.ticket.resolvedTickets);
  const loadingResolved = useSelector((state) => state.ticket.loadingResolved);
  ...
  const handleResolve = (ticket) => {
    ticket.resolved ? UnresolveTicket(ticket._id) : ResolveTicket(ticket._id);
  };
                ...
                <>
                  {tickets.length > 0 ? (
                        ...
                        {tickets.map((ticket, k) => (
                          <TableRow key={k}>
                            ...
                            <TableCell>
                              ...
                              {ticket.type === 2 && 'Time-off Request'}
                            </TableCell>
                            <TableCell>
                              <Button
                                variant='contained'
                    ...
                    <p style={{ marginTop: '1rem' }}>
                      There are currently no unresolved tickets.
                    </p>
                  )}
                </>
              )}
        ...
        <Box className='modal-container' sx={{ width: 400 }}>
          {tab === 0 && (
            <>
              ...
              <p>
                <strong>Name:</strong> {tickets[current]?.user.firstName}{' '}
                {tickets[current]?.user.lastName}
              </p>
              <p>
                <strong>Email:</strong> {tickets[current]?.user.email}
              </p>
              <p>
                <strong>Phone:</strong> {tickets[current]?.user.phone}
              </p>
              <p>
                <strong>Type:</strong>{' '}
                {tickets[current]?.type === 1 && 'Payroll Issue'}
                {tickets[current]?.type === 2 && 'Time-off Request'}
              </p>
              <Divider
          ...
          {tab === 1 && (
            <>
              ...
              <p>
                <strong>Email:</strong> {resolvedTickets[current]?.user.email}
              </p>
              <p>
                <strong>Phone:</strong> {resolvedTickets[current]?.user.phone}
              </p>
              <p>
                <strong>Type:</strong>{' '}
                {resolvedTickets[current]?.type === 1 && 'Payroll Issue'}
                {resolvedTickets[current]?.type === 2 && 'Time-off Request'}
              </p>
              <Divider
                sx={{ marginTop: '1rem', marginBottom: '1rem' }}
              ></Divider>
